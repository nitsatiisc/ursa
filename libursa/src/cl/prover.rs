...
use std::ops::Deref;
use cl::prover::mocks::blinded_credential_secrets;
//use ffi::cl::prover::mocks::_process_credential_signature;
...
pub struct ProofBuilder {
    ...
    gen_init_proofs: Vec<GenInitProof>,
    c_list: Vec<Vec<u8>>,
    tau_list: Vec<Vec<u8>>,
}
...
impl ProofBuilder {
    ...
    fn _finalize_non_revocation_proof_va(
        ...
        for (x, y) in init_proof
            ...
            .zip(init_proof.c_list_params.as_list()?.iter())
        {
            x_list.push(x.clone() + ch_num_z.clone() * y.clone());
        }
        */
